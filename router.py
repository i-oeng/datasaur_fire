...
def route_ticket(ticket_id, engine, ai_data):
    ...
    if ticket['country'] != 'Казахстан' or pd.isna(ticket['city']):
        ...
        client_address = f"{ticket['city']}, {ticket['street'] if pd.notna(ticket['street']) else ''}"
        client_coords = get_coordinates(client_address)

        if not client_coords:

            target_office = 'Астана'
            eligible_managers = apply_skills(managers_df[managers_df['unit_name'].str.contains('Астана', case=False)], ticket, ai_data)
        else:

            distances = []
            for _, office in offices_df.iterrows():
                office_addr = f"{office['office_name']}, {office['address']}"
                office_coords = get_coordinates(office_addr)
                if office_coords:
